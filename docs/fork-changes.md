...
2. **精确身份：** cleanup 校验 workspace name、canonical path、Child change id 和 base commit id；不能只按 name 删除。
3. **拓扑检查：** destructive cleanup 前确认 `@` 仍是记录的 Child change、parents 对应 `D0`、没有 live foreign descendants。
4. **保守失败：** path 缺失、name 被重绑、snapshot/forget 失败、change 不匹配或出现未知 descendant 时，保留 workspace 与磁盘数据。
5. **正确顺序：** 仅在验证后 `jj workspace forget`、abandon owned change、删除已确认 forget 的路径。

这些检查只保护一个 `worktree:true` Child，不是 workflow recovery ledger。
...
- 删除 workspace、abandon change 或移除 scratch 时：身份可证明才删，否则保留。

...
test("capture and cleanup refuse intermediate in-workspace symlink; target sentinel survives", async () => {
	const mod = await loadBackend();
	const { createJjWorktrees, diffJjWorktrees, cleanupJjWorktrees } = mod;
	const source = createTempJjSource("subagents-synth-inws-symlink-");
	const baseDir = mkdtempSync(join(tmpdir(), "subagents-synth-inws-symlink-base-"));
	const hookDir = mkdtempSync(join(tmpdir(), "subagents-synth-inws-symlink-bin-"));
	const hookPath = writeHook(
		hookDir,
		`#!/bin/sh
mkdir -p cache/tmp
...
`,
	...
	const plantInWorkspaceEscape = (worktreePath) => {
		const targetDir = join(worktreePath, ".inws-target");
		const sentinel = join(targetDir, "SENTINEL");
		rmSync(targetDir, { recursive: true, force: true });
		mkdirSync(targetDir, { recursive: true });
		writeFileSync(sentinel, "in-workspace-must-survive\n");
		const cachePath = join(worktreePath, "cache");
		rmSync(cachePath, { recursive: true, force: true });
		symlinkSync(targetDir, cachePath);
		return { targetDir, sentinel, cachePath };
	};
	const unlinkInWorkspaceEscape = (worktreePath) => {
		const cachePath = join(worktreePath, "cache");
		try {
			if (existsSync(cachePath) && lstatSync(cachePath).isSymbolicLink()) unlinkSync(cachePath);
		} catch {
			// Best-effort unlink of the intermediate link only.
		}
		rmSync(join(worktreePath, ".inws-target"), { recursive: true, force: true });
	};

	try {
		const captureSetup = createJjWorktrees(source, "synthInWsCap", 1, {
			baseDir,
			setupHook: { hookPath, timeoutMs: 10_000 },
		});
		const planted = plantInWorkspaceEscape(captureSetup.worktrees[0].path);
		const diffsDir = mkdtempSync(join(tmpdir(), "subagents-synth-inws-escape-diffs-"));
		try {
			const diffs = diffJjWorktrees(captureSetup, ["agent-a"], diffsDir);
			assert.match(diffs[0]?.error ?? "", /intermediate symlink|escapes the JJ workspace/i);
			assert.ok(existsSync(planted.sentinel), "capture must not delete the in-workspace target sentinel");
			assert.equal(readFileSync(planted.sentinel, "utf8"), "in-workspace-must-survive\n");
			assert.ok(existsSync(planted.targetDir), "capture must leave the in-workspace symlink target intact");
		} finally {
			unlinkInWorkspaceEscape(captureSetup.worktrees[0].path);
			rmSync(diffsDir, { recursive: true, force: true });
			cleanupJjWorktrees(captureSetup, {
				kind: "discard",
				authorization: { kind: "policy" },
		...
		const cleanupSetup = createJjWorktrees(source, "synthInWsClean", 1, {
			baseDir,
			setupHook: { hookPath, timeoutMs: 10_000 },
		});
		const cleanupPlanted = plantInWorkspaceEscape(cleanupSetup.worktrees[0].path);
		try {
			const report = cleanupJjWorktrees(cleanupSetup, {
				kind: "discard",
				authorization: { kind: "policy" },
			...
			assert.ok(existsSync(cleanupSetup.worktrees[0].path));
			assert.ok(
				existsSync(cleanupPlanted.sentinel),
				"cleanup must not delete the in-workspace target sentinel",
			);
			assert.equal(readFileSync(cleanupPlanted.sentinel, "utf8"), "in-workspace-must-survive\n");
		} finally {
			unlinkInWorkspaceEscape(cleanupSetup.worktrees[0].path);
			if (existsSync(cleanupSetup.worktrees[0].path)) {
				cleanupJjWorktrees(cleanupSetup, {
					kind: "discard",
					authorization: { kind: "policy" },

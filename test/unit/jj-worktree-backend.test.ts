...
// This fork owns the backend in source; retain the old name for staging helpers below.
const overlayInstalled = overlaySource;
...
async function loadBackend() {
	...
	assert.equal(
		createHash("sha256").update(readFileSync(overlayInstalled)).digest("hex"),
		createHash("sha256").update(readFileSync(overlaySource)).digest("hex"),
	);
...
function makeJjPathWrapper(mode) {
	// mode: 'fail-forget' | 'fail-update-stale' | 'fail-edit' | 'fail-abandon' | 'pass'
...
test("Git fallback: isJjWorktreeCwd false; stock worktree path remains Git", async () => {
	const mod = await loadBackend();
	...
	try {
		...
		const worktree = readFileSync(
			join(root, "src/runs/shared/worktree.ts"),
			"utf8",
		);
...
`,
	...
	try {
		...
		for (const id of after) {
			// Allow Source @ mutation from snapshot; only reject extra non-before commits
			// beyond a single possible Source WC rewrite is hard — instead assert no
			// abandoned-orphan: every visible commit was either present before or is current @.
		}
...
test("JJ recovery text uses jj commands not git", async () => {
	const handoffSrc = readFileSync(
		join(root, "src/runs/shared/parallel-handoff.ts"),
		"utf8",
	);
	assert.match(handoffSrc, /jj -R/);
	assert.match(handoffSrc, /workspace forget/);
	// Inside the jj branch, no git -C
	const jjBranch = handoffSrc.split('if (task.backend === "jj")')[1]?.split("} else {")[0] ?? "";
	assert.ok(jjBranch.includes("jj -R"));
	assert.ok(!jjBranch.includes("git -C"));
});

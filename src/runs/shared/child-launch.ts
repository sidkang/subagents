...
function childProcessEnv(input: BuildInProcessChildLaunchInput, toolPlan: PiLaunchToolPlan): Record<string, string | undefined> {
	...
	// Only a detached runner applies env. A parent-hosted session uses the
	// binding-capturing hook below and must not change the parent's process.env.
	env[WORKFLOW_SCRATCH_ROOT_ENV] = getActiveWorkflowScratchLaunchBinding()?.hostRoot;

...
/**
 * Delete one synthetic path without following an intermediate symlink out of the
 * JJ workspace. The final symlink itself may be unlinked. Fail closed before any
 * external deletion.
 */
function removeSyntheticPath(worktreePath: string, syntheticPath: string): void {
	...
	for (let index = 0; index < parts.length; index++) {
		...
		if (stat.isSymbolicLink()) {
			...
			try {
				targetReal = fs.realpathSync(current);
			} catch (error) {
				const detail = error instanceof Error ? error.message : String(error);
				throw new Error(
					`synthetic path escapes the JJ workspace via intermediate symlink: ${syntheticPath} (${detail})`,
				);
			}
			if (!isInsideResolvedRoot(targetReal, workspaceRoot)) {
				throw new Error(

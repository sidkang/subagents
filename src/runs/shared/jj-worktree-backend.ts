 ...
 *    no live descendants → forget + abandon D + rm path
 *
...
export interface WorktreeInfo {
	...
	/** Exact provisioned workspace change id (cleanup identity). */
	workspaceChangeId?: string;
	/** Exact provisioned workspace commit id (forensics). */
...
/** Best-effort abandon when no live descendants (owned D residue). */
function abandonRevisionIfSafe(sourceRoot: string, changeOrCommitId: string): void {
	const id = changeOrCommitId.trim();
	if (!id) return;
...
function duplicateOwnedBase(root: string, sourceBaseCommit: string): {
	...
	if (!parentCommitIds) {
		abandonRevisionIfSafe(root, duplicateChangeId);
...
function hasTrackedEntries(worktreePath: string, relativePath: string): boolean {
	const result = runJj(worktreePath, ["file", "list", "-r", "@", relativePath]);
...
/**
 * Query live descendants of an exact recorded revision (commit or change id).
 * Fail closed on empty id, query failure, or ambiguity — callers must preserve.
 */
function liveDescendantsOfRevision(
	...
	if (!id) {
		return { ok: false, error: "descendants query refused: empty revision" };
	}
...
function taskBase(worktree: WorktreeInfo): Pick<WorktreeCleanupTask, "index" | "path" | "branch" | "backend" | "workspaceChangeId"> {
	return {
...
function rollbackOwnedWorkspace(
	...
	// Foreign descendant of owned W: never forget/abandon (would rewrite foreign history).
	const wDesc = liveDescendantsOfRevision(sourceRoot, recorded.changeId);
	if (!wDesc.ok) {
		errors.push(
	...
	if (wDesc.ids.length > 0) {
		errors.push(
			`setup-rollback refused: workspace change ${recorded.changeId} has live descendants (${wDesc.ids.join(", ")}); preserved registration/path`,
		);
		return { pathRemoved: false, forgotten: false, abandoned: false, errors };
	}
	...
	let abandoned = false;
	if (recorded?.changeId) {
		const abandon = runJj(sourceRoot, ["abandon", recorded.changeId]);
...
function createSingleJjWorktree(
	...
	if (add.status !== 0) {
		...
		// Best-effort abandon D if add failed cleanly.
		if (!pathExists && !nameAppeared) {
			abandonRevisionIfSafe(repo.root, dChange);
		}
		if (pathExists || nameAppeared) {
			throw new Error(
	...
	try {
		...
		rollbackOwnedWorkspace(repo.root, workspaceName, resolvedPath, {
			...
			// Parent check uses D0 parents via workspaceCommitId during normal cleanup;
			// setup-rollback uses recorded change abandon after forget.
			baseCommit: d0,
		});
		// If forget left D visible, abandon D.
		abandonRevisionIfSafe(repo.root, dChange);
		throw error;
	}
}
...
function removeSyntheticPaths(worktree: WorktreeInfo): void {
	...
	for (const syntheticPath of worktree.syntheticPaths) {
		...
		try {
			...
			if (stat.isSymbolicLink() || !stat.isDirectory()) fs.unlinkSync(resolved);
...
function cleanupSingleJjWorktree(
	...
	// Foreign descendant of owned W: preserve registration/path; never rewrite foreign history.
	if (worktree.workspaceChangeId) {
		const wDesc = liveDescendantsOfRevision(setup.cwd, worktree.workspaceChangeId);
		if (!wDesc.ok) {
			return {
				...base,
				worktreeRemoved: false,
		...
		if (wDesc.ids.length > 0) {
			return {
				...
				errors: [
					...errors,
					`cleanup refused: workspace change ${worktree.workspaceChangeId} has live descendants (${wDesc.ids.join(", ")}); preserved registration/path`,
				],
			};
		}
	}
	...
	// Abandon only the recorded change id.
	if (!worktree.workspaceChangeId) {
		...
		return {
			...
			branchRemoved: false,
			preserved: true,
			reason: "missing workspaceChangeId",
			errors: [...errors, "cleanup refused abandon: missing recorded workspaceChangeId"],
		};
	}
	const abandon = runJj(setup.cwd, ["abandon", worktree.workspaceChangeId]);
	if (abandon.status !== 0 && !isMissingRevision(abandon.stderr, abandon.stdout)) {

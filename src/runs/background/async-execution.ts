...
import { disableWorkflowScratchCleanup, getActiveWorkflowScratchLaunchBinding, WORKFLOW_SCRATCH_ROOT_ENV } from "../shared/workflow-scratch.ts";
...
function isStaleExtensionContextError(error: unknown): boolean {
	// Fork contract: only Pi's explicit stale-context error is ignorable.
...
function spawnRunner(cfg: object, suffix: string, cwd: string, initialStatus: Omit<AsyncStatus, "pid" | "processTerminal">, initialStatusPath: string, launchParentSessionId: string | undefined, onProcessTerminal?: (proof: unknown) => void, onBeforeProceed?: (runnerProcessInstanceId: string) => void, requestedCwd = cwd): SpawnRunnerResult | Promise<SpawnRunnerResult> {
	...
	// This is the actual detach boundary, including implicit async defaults and
	// resumed runs. A stop request is not proof that the runner has stopped.
	if (workflowScratchBinding) disableWorkflowScratchCleanup();
	const launchConfig = { ...cfg, runnerProcessInstanceId, ...(launchBarrierToken ? { launchBarrierToken } : {}), ...(workflowScratchBinding ? { workflowScratchBinding } : {}) };
